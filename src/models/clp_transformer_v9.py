...
class CLPTransformer(Transformer):
    def __init__(self, block_dim, action_dim, space_dim, d_model=64, nhead=4, num_layers=3, ff_dim_multiplier=3, dropout=0.1):
        ...
        # --- COMPRESIÓN DE INVENTARIO EN EL ENCODE ---
    ...
    def encode(self, block_features):
        ...
        # Cross-Attention para extraer el resumen macro del catálogo
        ...
        # Retroproyección por suma residual: los bloques absorben el inventario aquí
        enriched_memory = raw_memory + inv_summary
        
        return enriched_memory, inv_summary
    ...
    def decode(self, memory, inv_summary, action_blocks, action_features, placed_features, space_features):
        ...
        # 3. CROSS-ATTENTION CONTEXTUAL LINEAL (🚨 QUERY PURA DEL ESPACIO)
        # Eliminamos la query_fusion. El espacio interroga directamente al contenedor construido.
        enriched_space, _ = self.cross_attention_block(
            query=space_emb, # Query directo, limpio y geométrico
            key=keys,
            value=values,
        ...
        # El gather recupera los bloques que YA contienen la información del inventario
