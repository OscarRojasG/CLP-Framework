...
class CLPTransformer(Transformer):
    def __init__(self, block_dim, action_dim, placed_dim, space_dim, d_model=256, nhead=8, num_layers=3, ff_dim_multiplier=4, dropout=0.1):
        ...
        # Nueva Atención Cruzada: Contexto -> Memoria Global de Bloques (Fusión Ligera)
        self.fussion_cross_att = CrossAttentionDecoder(d_model, nhead, ff_dim_multiplier, dropout, num_layers=1)

        # Proyecciones para el Scaled Dot-Product final entre el estado fusionado y las acciones
    ...
    def decode(self, memory, action_blocks, action_features, placed_blocks, placed_features, space_features):
        ...
        # 5. NUEVO: ATENCIÓN CRUZADA CONTEXTO → MEMORIA GLOBAL DE BLOQUES
        # Q = ctx [B, 1, d_model]
        # K, V = memory [B, N_blocks, d_model] (Catálogo de bloques libres disponible)
        # El contexto geométrico evalúa toda la memoria libre para refinar su estado de representación
        fused_ctx = self.fussion_cross_att(ctx, memory, memory) # [B, 1, d_model]

        # ---------------------------------------------------
        # 6. SCALED DOT-PRODUCT DIRECTO (Contexto Fusionado vs Acciones)
        # ---------------------------------------------------
        # Proyectamos al espacio de comparación
        q = self.q_proj(fused_ctx)    # [B, 1, d_model]
