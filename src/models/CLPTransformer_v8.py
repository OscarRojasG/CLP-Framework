...
class CLPTransformer(BiasedTransformer):
    ...
    def decode(self, memory, action_blocks, action_features, placed_blocks, placed_features, space_features, bias):     
        B = memory.shape[0]
        ...
        # 6.1 Logits base del Transformer
        logits = self.output(attn_out).squeeze(-1)  # [B, Na]

        # 6.2 Normalización Z-score "Mask-aware"
        # Convertimos la máscara a float (1.0 para real, 0.0 para padding)
        mask_float = action_mask.type(bias.dtype)
        
        # Calculamos la media solo de los elementos válidos
        # Sumamos los valores válidos y dividimos por el conteo de elementos válidos
        count = mask_float.sum(dim=-1, keepdim=True).clamp(min=1.0)
        mu = (bias * mask_float).sum(dim=-1, keepdim=True) / count
        
        # Calculamos la varianza solo de los elementos válidos
        # Var = E[X^2] - (E[X])^2
        diff_sq = (((bias - mu) ** 2) * mask_float).sum(dim=-1, keepdim=True)
        std = torch.sqrt(diff_sq / count + 1e-6)

        # Aplicamos la normalización
        bias_normalized = (bias - mu) / std

        # 6.3 Transformación a Log-Space (Heurística)
        # Llenamos el padding con -inf ANTES del log_softmax para que su prob sea 0.0
        bias_for_softmax = bias_normalized.masked_fill(~action_mask, float('-inf'))
        h_bias = torch.nn.functional.log_softmax(bias_for_softmax, dim=-1)

        # 6.4 Fusión Final (Transformer + Heurística)
        # Sumamos las dos opiniones en espacio logarítmico
        logits = logits + h_bias
        
        # 6.5 Máscara de seguridad final
        # Forzamos -inf en el padding para el CrossEntropyLoss del entrenamiento
