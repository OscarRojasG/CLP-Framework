...
class BlockEncoder(nn.Module):
    def __init__(self, src_dim, d_model, num_layers=2):
        super().__init__()
        layers = []
        input_dim = src_dim
...
class ActionEncoder(nn.Module):
    def __init__(self, action_dim, d_model, num_layers=2):
        super().__init__()
        layers = []
...
class Transformer(EncoderDecoderPEModel):
    def __init__(self, src_dim, tgt_dim, d_model=256, nhead=8, decoder_num_layers=3, dropout=0.1,
                 block_encoder_layers=2, action_encoder_layers=2):
        super().__init__()
        self.d_model = d_model

        # Componentes principales con capas dinámicas
        self.block_encoder = BlockEncoder(src_dim, d_model, num_layers=block_encoder_layers)
        self.action_encoder = ActionEncoder(tgt_dim - 1, d_model, num_layers=action_encoder_layers)
