...
class InputAdapterV4(InputAdapter):
    ...
    def enc_2_vec(self, blocks: list[Block]):
        block_features = np.full((self.max_blocks, 4), -1, dtype=np.float32)

        n_b = len(blocks)
        block_features[:n_b] = [[b.l, b.w, b.h, b.volume()] for b in blocks[:n_b]]
    ...
    def dec_2_vec(self, blocks: list[Block], space: Space, pblocks: list[PBlock], actions: list[Action]):
        ...
        vcs[:n_a] = [a.norm_vcs() for a in actions]
