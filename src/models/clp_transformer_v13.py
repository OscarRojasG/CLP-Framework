...
class CLPTransformer(Transformer):
    def __init__(self, block_dim, action_dim, space_dim, d_model=64, nhead=4, num_layers=3, ff_dim_multiplier=3, dropout=0.1):
        ...
        self.summary_dropout = nn.Dropout(dropout) # 🚨 NUEVO: Dropout para regularizar la inyección global
        ...
        self.attn_dropout = nn.Dropout(dropout) # 🚨 NUEVO: Dropout para la salida de la Cross-Attention
    ...
    def encode(self, block_features):
        ...
        contextual_modifier = self.summary_proj(inv_summary) # [B, 1, d_model]

        # 🚨 INCORPORADO: Se regulariza el modificador antes de aplicarlo residualmente al catálogo
    ...
    def decode(self, memory, action_blocks, action_features, placed_features, space_features):
        ...
        for i in range(self.num_layers):
            ...
            # 🚨 INCORPORADO: Dropout clásico de la subcapa de atención antes del Post-LN residual
