import torch
import torch.nn as nn
import torch.nn.functional as F
from models.base.mlp_encoder import MLPEncoder
from models.base.transformer import Transformer

class CLPTransformer(Transformer):
    def __init__(self, block_dim, action_dim, space_dim, placed_dim, d_model=64, nhead=4, num_layers=3, ff_dim_multiplier=3, dropout=0.1):
        super().__init__(
            block_dim=block_dim, action_dim=action_dim, space_dim=space_dim, placed_dim=placed_dim,
            d_model=d_model, nhead=nhead, num_layers=num_layers,
            ff_dim_multiplier=ff_dim_multiplier, dropout=dropout
        )
        self.d_model = d_model
        self.num_layers = num_layers

        # 1. Proyecciones base modificadas
        self.block_proj = nn.Linear(block_dim, d_model)
        self.placed_proj = nn.Linear(placed_dim, d_model)

        # MEJORA 1: MLP de fusión para la Query en lugar de doble suma lineal 🚨
        # Recibe: d_model (del bloque) + action_dim (features) + space_dim (coordenadas)
        self.query_fusion_mlp = nn.Sequential(
            nn.Linear(d_model + action_dim + space_dim, d_model * ff_dim_multiplier),
            nn.ReLU(),
            nn.Linear(d_model * ff_dim_multiplier, d_model),
            nn.Dropout(dropout)
        )

        # Encoders basados en MLP para catálogo
        self.block_encoder = MLPEncoder(d_model, ff_dim_multiplier, dropout, num_layers)

        # --- ENCODER POOLING (INVENTARIO) ---
        self.inv_query_token = nn.Parameter(torch.randn(1, 1, d_model))
        self.inv_pooling_attn = nn.MultiheadAttention(embed_dim=d_model, num_heads=nhead, dropout=dropout, batch_first=True)
        self.summary_proj = nn.Linear(d_model, d_model)
        self.summary_dropout = nn.Dropout(dropout)
        self.norm_enrich = nn.LayerNorm(d_model)

        # --- BLOQUE DE SELF-ATTENTION DE BLOQUES COLOCADOS (Derecha) ---
        self.placed_self_attn = nn.MultiheadAttention(embed_dim=d_model, num_heads=nhead, dropout=dropout, batch_first=True)
        self.placed_norm1 = nn.LayerNorm(d_model) # Pre-LN
        self.placed_norm2 = nn.LayerNorm(d_model) # 🚨 MEJORA 3: Post-Attention Norm para estabilidad
        self.placed_dropout = nn.Dropout(dropout)

        # --- TOKEN VACÍO DE SEGURIDAD ANTI-NaNs ---
        self.empty_token = nn.Parameter(torch.randn(1, 1, d_model))

        # --- BLOQUE DE CROSS-ATTENTION COMPLETO (Arriba) ---
        self.cross_attn_layers = nn.ModuleList([
            nn.MultiheadAttention(embed_dim=d_model, num_heads=nhead, dropout=dropout, batch_first=True)
            for _ in range(num_layers)
        ])
        self.decoder_ff_layers = nn.ModuleList([
            nn.Sequential(
                nn.Linear(d_model, ff_dim_multiplier * d_model),
                nn.ReLU(),
                nn.Linear(ff_dim_multiplier * d_model, d_model),
                nn.Dropout(dropout)
            )
            for _ in range(num_layers)
        ])
        self.attn_dropout = nn.Dropout(dropout)

        self.dec_norm1_layers = nn.ModuleList([nn.LayerNorm(d_model) for _ in range(num_layers)])
        self.dec_norm2_layers = nn.ModuleList([nn.LayerNorm(d_model) for _ in range(num_layers)])
        self.final_dec_norm = nn.LayerNorm(d_model)

        # MEJORA 2: Cabezal de energía lineal para proyección de logits finales 🚨
        self.energy_proj = nn.Linear(d_model, 1)

    def encode(self, block_features):
        B, N_blocks, _ = block_features.shape
        block_padding_mask = torch.all(block_features == -1.0, dim=-1)

        x = self.block_proj(block_features)
        raw_memory = self.block_encoder(x.view(-1, self.d_model)).view(B, N_blocks, self.d_model)

        query = self.inv_query_token.expand(B, -1, -1)
        inv_summary, _ = self.inv_pooling_attn(
            query=query, key=raw_memory, value=raw_memory, key_padding_mask=block_padding_mask
        )

        contextual_modifier = self.summary_proj(inv_summary)
        contextual_modifier = self.summary_dropout(contextual_modifier)

        enriched_memory = self.norm_enrich(raw_memory + contextual_modifier)
        return enriched_memory,

    def decode(self, memory, action_blocks, action_features, space_features, placed_blocks, placed_features):
        B = memory.shape[0]
        Na = action_blocks.shape[1]
        Np = placed_blocks.shape[1]

        # --- 1. CONSTRUCCIÓN DE LA QUERY CON CONCATENACIÓN (MEJORA 1) 🚨 ---
        action_mask = (action_blocks != -1)
        action_idx_clamped = action_blocks.clamp(min=0)
        
        # Mapeo del embedding del bloque desde memoria
        block_emb_action = torch.gather(memory, 1, action_idx_clamped.unsqueeze(-1).expand(-1, -1, self.d_model))
        
        # Expandimos el espacio único para que se acople a cada acción candidato [B, 1, 6] -> [B, Na, 6]
        if len(space_features.shape) == 2:
            space_features = space_features.unsqueeze(1)
        space_features_expanded = space_features.expand(-1, Na, -1)
        
        # Concatenamos las características brutas en el último eje
        raw_query_concat = torch.cat([block_emb_action, action_features, space_features_expanded], dim=-1)
        
        # El MLP procesa las interacciones no lineales completas
        queries = self.query_fusion_mlp(raw_query_concat) # [B, Na, d_model]

        # --- 2. CONSTRUCCIÓN DE KEYS/VALUES MASCARADAS (Ruta Derecha) ---
        placed_mask = (placed_blocks != -1)
        placed_idx_clamped = placed_blocks.clamp(min=0)
        
        block_emb_placed = torch.gather(memory, 1, placed_idx_clamped.unsqueeze(-1).expand(-1, -1, self.d_model))
        placed_feat_emb = self.placed_proj(placed_features)
        placed_sequence = block_emb_placed + placed_feat_emb # [B, Np, d_model]

        # Máscara de multiplicación por cero anti-ruido en LayerNorm
        mask_expanded = placed_mask.unsqueeze(-1).expand(-1, -1, self.d_model)
        placed_sequence = placed_sequence * mask_expanded

        # Inyección del empty token
        dummy_placed_token = self.empty_token.expand(B, -1, -1)
        unified_placed_sequence = torch.cat([dummy_placed_token, placed_sequence], dim=1) # [B, 1 + Np, d_model]

        dummy_mask = torch.zeros((B, 1), dtype=torch.bool, device=placed_blocks.device)
        extended_placed_mask = torch.cat([dummy_mask, ~placed_mask], dim=1)

        # Flujo de Self-Attention
        normed_placed = self.placed_norm1(unified_placed_sequence)
        keys_values, _ = self.placed_self_attn(
            query=normed_placed, key=normed_placed, value=normed_placed, key_padding_mask=extended_placed_mask
        )
        # Residuos + MEJORA 3 (Post-Attention Norm para calibrar la escala latente) 🚨
        keys_values = self.placed_norm2(unified_placed_sequence + self.placed_dropout(keys_values))

        # --- 3. PROCESAMIENTO CRUZADO (Cross-Attention Superior) ---
        for i in range(self.num_layers):
            normed_queries = self.dec_norm1_layers[i](queries)
            attn_output, _ = self.cross_attn_layers[i](
                query=normed_queries, key=keys_values, value=keys_values, key_padding_mask=extended_placed_mask 
            )
            queries = queries + self.attn_dropout(attn_output)

            normed_queries_2 = self.dec_norm2_layers[i](queries)
            ff_output = self.decoder_ff_layers[i](normed_queries_2)
            queries = queries + ff_output

        queries = self.final_dec_norm(queries)

        # --- 4. EXTRACCIÓN DE LOGITS MEDIANTE PROYECTOR DE ENERGÍA (MEJORA 2) 🚨 ---
        energy_scores = self.energy_proj(queries) # [B, Na, 1]
        logits = energy_scores.squeeze(-1)         # [B, Na]
        
        logits = logits.masked_fill(~action_mask, float('-inf'))

        return logits